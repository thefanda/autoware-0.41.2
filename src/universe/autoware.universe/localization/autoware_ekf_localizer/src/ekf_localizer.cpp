  ...
  twist_queue_(params_.twist_smoothing_steps)
{
  ...
  sub_initialpose_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "initialpose", 1, std::bind(&EKFLocalizer::callback_initial_pose, this, _1));
        gnss_sub_initialpose_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "/sensing/gnss/pose", 1, std::bind(&EKFLocalizer::callback_gnss, this, _1)); 
  sub_location_imu = create_subscription<dfcv_mining_msgs::msg::LocationIMU>(
            "/location_imu_info", 1,
            std::bind(&EKFLocalizer::imucallback, this, _1));
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
  ...
  std::cout << "is_converged2222222: " << std::endl;
  pose_queue_.push(msg);

  publish_callback_return_diagnostics("pose", msg->header.stamp);
}
void EKFLocalizer::imucallback(const dfcv_mining_msgs::msg::LocationIMU::SharedPtr msg){

  twist_cov_.header = msg->header;
  twist_cov_.twist.twist.linear.x = msg->vx;
  twist_cov_.twist.twist.linear.y = 0.0;
  twist_cov_.twist.twist.linear.z = 0.0;
  twist_cov_.twist.twist.angular.x = 0.0;
  twist_cov_.twist.twist.angular.y = 0.0;
  twist_cov_.twist.twist.angular.z = msg->gyro_z*3.1415/180;
  for (int i = 0; i < 36; ++i) twist_cov_.twist.covariance[i] = 0.5;


}
void EKFLocalizer::callback_gnss(const geometry_msgs::msg::PoseStamped::SharedPtr gnss_msg)
{

  return;
  // PoseWithCovariance
  geometry_msgs::msg::PoseWithCovarianceStamped pose_cov;
  pose_cov.header = gnss_msg->header;
  pose_cov.pose.pose = gnss_msg->pose;
  for (int i = 0; i < 36; ++i) pose_cov.pose.covariance[i] = 0.5;  // 简单假设 GNSS 精度

  // TwistWithCovariance
  // geometry_msgs::msg::TwistWithCovarianceStamped twist_cov;
  // twist_cov.header = gnss_msg->header;
  // twist_cov.twist.twist.linear.x = 0.0;
  // twist_cov.twist.twist.linear.y = 0.0;
  // twist_cov.twist.twist.linear.z = 0.0;
  // twist_cov.twist.twist.angular.x = 0.0;
  // twist_cov.twist.twist.angular.y = 0.0;
  // twist_cov.twist.twist.angular.z = 0.0;
  // for (int i = 0; i < 36; ++i) twist_cov.twist.covariance[i] = 0.5;

  // Odometry
  nav_msgs::msg::Odometry odom;
  odom.header = gnss_msg->header;
  odom.header.frame_id = "map";
  odom.child_frame_id = "base_link";
  odom.pose = pose_cov.pose;
  odom.twist = twist_cov_.twist;
    geometry_msgs::msg::TwistStamped twist_msg;
    twist_msg.header = twist_cov_.header;
    twist_msg.twist = twist_cov_.twist.twist;
pub_twist_->publish(twist_msg);

  // 发布
  pub_pose_->publish(*gnss_msg);
  pub_pose_cov_->publish(pose_cov);
  pub_twist_cov_->publish(twist_cov_);
  pub_odom_->publish(odom);

  // 发布 TF
  geometry_msgs::msg::TransformStamped tf_msg;
  tf_msg.header = gnss_msg->header;
  tf_msg.child_frame_id = "base_link";
  tf_msg.header.frame_id = "map";
  tf_msg.transform.translation.x = gnss_msg->pose.position.x;
  tf_msg.transform.translation.y = gnss_msg->pose.position.y;
  tf_msg.transform.translation.z = gnss_msg->pose.position.z;
  tf_msg.transform.rotation = gnss_msg->pose.orientation;
