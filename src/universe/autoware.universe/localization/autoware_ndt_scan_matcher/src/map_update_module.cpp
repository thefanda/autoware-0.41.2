  ...
  std::unique_ptr<DiagnosticsInterface> & diagnostics_ptr)
{
  ...
  //   RCLCPP_INFO(logger_, "original map size: %zu points", cloud->size());

  //   // -------------------------------
  //   // ★★★ Begin: VoxelGrid Downsample
  //   // -------------------------------
  //   pcl::VoxelGrid<PointTarget> sor;
  //   sor.setInputCloud(cloud);
   
  //   // leaf_size 你可以放到参数 param_.voxel_leaf_size
  //   float leaf = 0.3f;   // 0.5米建议值（Autoware NDT 默认地图分辨率 1.0m）
  //   sor.setLeafSize(leaf, leaf, leaf);

  //   auto filtered = pcl::make_shared<pcl::PointCloud<PointTarget>>();
  //   sor.filter(*filtered);

  //   RCLCPP_INFO(
  //     logger_,
  //     "voxel filtered size: %zu points (leaf=%.2f)",
  //     filtered->size(), leaf);

  //   // 替换 cloud 为 filtered 进入 NDT
  //   cloud = filtered;
  //   // -------------------------------
  //   // ★★★ End: VoxelGrid Downsample
  //   // -------------------------------
  //    pcl::io::savePCDFileBinary("/autoware/a.pcd", *cloud); 
  //   // 原本的添加逻辑
  ...
  for (auto & map : maps_to_add) {
    ...
    pcl::fromROSMsg(map.pointcloud, *cloud);
      RCLCPP_INFO(logger_, "map point cloud size: %zu points", 
            cloud->size());
