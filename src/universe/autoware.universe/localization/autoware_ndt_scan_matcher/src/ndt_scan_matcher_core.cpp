  ...
  sensor_msgs::msg::PointCloud2::ConstSharedPtr sensor_points_msg_in_sensor_frame)
{
  ...
  RCLCPP_INFO(this->get_logger(), "NDT InputSource point cloud size: %zu points", 
            sensor_points_in_baselink_frame->size());
  // check is_activated
...
RCLCPP_INFO(this->get_logger(), "NDT align time: %.3f ms", duration_ms);
  ...
  tier4_localization_msgs::srv::PoseWithCovarianceStamped::Response::SharedPtr res)
{
  ...
  try {
    transform_s2t = tf2_buffer_.lookupTransform(target_frame, source_frame, tf2::TimePointZero);
     RCLCPP_INFO(this->get_logger(),
        "Transform from %s to %s: translation = [%.3f, %.3f, %.3f]",
        source_frame.c_str(), target_frame.c_str(),
        transform_s2t.transform.translation.x,
        transform_s2t.transform.translation.y,
        transform_s2t.transform.translation.z);
         tf2::Quaternion q(
        transform_s2t.transform.rotation.x,
        transform_s2t.transform.rotation.y,
        transform_s2t.transform.rotation.z,
        transform_s2t.transform.rotation.w);
    double roll, pitch, yaw;
    tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);
    RCLCPP_INFO(this->get_logger(),
        "Rotation RPY = [roll=%.3f, pitch=%.3f, yaw=%.3f]",
        roll, pitch, yaw);
  ...
  //const auto & pos = initial_pose_msg_in_map_frame.pose.pose.position;
  // RCLCPP_INFO(this->get_logger(),
  //     "Pose in map_frame: x=%.3f, y=%.3f, z=%.3f",
  //     pos.x, pos.y, pos.z);

  // // 打印旋转信息（四元数）
  // const auto & ori = initial_pose_msg_in_map_frame.pose.pose.orientation;
  // RCLCPP_INFO(this->get_logger(),
  ...
  // tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);
  // RCLCPP_INFO(this->get_logger(),
  //     "Orientation RPY (rad): roll=%.3f, pitch=%.3f, yaw=%.3f",
  //     roll, pitch, yaw);
  ...
  const geometry_msgs::msg::PoseWithCovarianceStamped & initial_pose_with_cov)
{
  ...
  for (int64_t i = 0; i < param_.initial_pose_estimation.particles_num; i++) {
    ...
    // initial_pose.position.x = initial_pose_with_cov.pose.pose.position.x;
    // initial_pose.position.y = initial_pose_with_cov.pose.pose.position.y;
    ...
    // 用 Matrix4f 构造 Affine3f（这是 getEulerAngles 能接受的）
    Eigen::Affine3f aff(initial_pose_matrix);

    float roll, pitch, yaw;

    // PCL需要 Transform/Affine，因此用 aff 而不是 rot
    pcl::getEulerAngles(aff, roll, pitch, yaw);

    RCLCPP_INFO(this->get_logger(),
        "Initial Pose RPY(deg): roll=%.2f pitch=%.2f yaw=%.2f",
        roll * 180.0f / M_PI,
        pitch * 180.0f / M_PI,
        yaw * 180.0f / M_PI);
