  ...
  gnss_pose_pub_method_(static_cast<int>(declare_parameter<int>("gnss_pose_pub_method")))
{
  ...
  nav_sat_fix_sub_ = create_subscription<dfcv_mining_msgs::msg::LocationIMU>(
    "/location_imu_info", rclcpp::QoS{1},
...
void GNSSPoser::callback_nav_sat_fix(
  const dfcv_mining_msgs::msg::LocationIMU::ConstSharedPtr nav_sat_fix_msg_ptr)
{
  ...
  // check fixed topic
 const bool is_status_fixed = (nav_sat_fix_msg_ptr->gps_week_navigation == 4);
  ...
  gps_point.latitude = nav_sat_fix_msg_ptr->lat;
  gps_point.longitude = nav_sat_fix_msg_ptr->lon;
  gps_point.altitude = nav_sat_fix_msg_ptr->hil_z;
  ...
  position.z = nav_sat_fix_msg_ptr->hil_z;
  ...
  tf2::Quaternion q;
  q.setRPY(nav_sat_fix_msg_ptr->roll*M_PI/180, -nav_sat_fix_msg_ptr->pitch*M_PI/180, (90-nav_sat_fix_msg_ptr->yaw-3.9)*M_PI/180);
  gnss_antenna_pose.orientation = tf2::toMsg(q);
  ...
  gnss_base_pose_cov_msg.pose.covariance[0] = 0.01; // x
  gnss_base_pose_cov_msg.pose.covariance[7] = 0.01; // y
  gnss_base_pose_cov_msg.pose.covariance[14] = 0.01; // z

  // 如果你有 INS orientation rmse，可以填充旋转协方差
  if (use_gnss_ins_orientation_) {
      constexpr double yaw_std_dev_deg = 0.1;
      constexpr double yaw_std_dev_rad = yaw_std_dev_deg * M_PI / 180.0;
      gnss_base_pose_cov_msg.pose.covariance[35] = std::pow(yaw_std_dev_rad, 2); // 3.0e-6
      // roll、pitch 同理
      gnss_base_pose_cov_msg.pose.covariance[21] = std::pow(yaw_std_dev_rad, 2);
      gnss_base_pose_cov_msg.pose.covariance[28] = std::pow(yaw_std_dev_rad, 2);
  } else {
    gnss_base_pose_cov_msg.pose.covariance[21] = 0.00000001;
    gnss_base_pose_cov_msg.pose.covariance[28] = 0.00000001;
    gnss_base_pose_cov_msg.pose.covariance[35] = 0.00000001;
  }
