...
This node is a CUDA accelerated version of the `PolarVoxelOutlierFilter` available in [autoware_pointcloud_preprocessor](../../autoware_pointcloud_preprocessor).

## Inner-workings / Algorithms

This node is an alternative implementation to `autoware::pointcloud_preprocessor_v49::PolarVoxelOutlierFilterComponent`, which filters outliers based on voxels in polar coordinate space instead of Cartesian coordinate space.
...
See [the original implementation in autoware_pointcloud_preprocessor](../../autoware_pointcloud_preprocessor/docs/polar-voxel-outlier-filter.md) for the detail.
...
Due to differences in floating-point arithmetic between CPUs and GPUs, the outputs of `autoware::pointcloud_preprocessor_v49::PolarVoxelOutlierFilterComponent` and this filter may not be identical.
