...
#include "autoware/cuda_pointcloud_preprocessor_v49/cuda_outlier_filter/cuda_polar_voxel_outlier_filter.hpp"
#include "autoware/cuda_pointcloud_preprocessor_v49/device_optional.hpp"
...
struct ValidAndNotEqualTo
{
  ...
  __host__ __device__ bool operator()(
    const DeviceOptional<DataType> & lhs, const DeviceOptional<DataType> & rhs)
  {
...
struct NulloptToMax
{
  ...
  operator()(const DeviceOptional<DataType> & opt) const
  {
    return opt.has_value() ? opt.value() : std::numeric_limits<DataType>::max();
  }
};
...
struct NulloptToLowest
{
  ...
  operator()(const DeviceOptional<DataType> & opt) const
  {
    return opt.has_value() ? opt.value() : std::numeric_limits<DataType>::lowest();
  }
};
...
{
  return (field_index == FieldDataIndex::radius)
           ? fabs(static_cast<double>(field_data)) >= static_cast<double>(std::numeric_limits<T>::epsilon())
           : true;
}
...
__device__ void assign_polar_index(
  ...
  FieldDataComposer<DeviceOptional<int32_t> *> & outputs)
{
  ...
  if (!is_finite || !is_within_radius_range || !has_sufficient_radius) {
    ...
    output[point_index] = nullopt;
    return;
  }

  if constexpr (std::is_same<TFieldData, double>::value) {
  ...
  FieldDataComposer<DeviceOptional<int32_t> *> outputs)
{
  ...
  if constexpr (std::is_same<TPolarData, double>::value) {
    switch (field_index) {
...
__global__ void calculate_voxel_index_kernel(
  const FieldDataComposer<DeviceOptional<int32_t> *> field_indices, const size_t num_points,
  const FieldDataComposer<int> field_dimensions, const FieldDataComposer<int> field_mins,
  DeviceOptional<int> * point_indices, DeviceOptional<int> * voxel_indices)
{
  auto point_index = blockIdx.x * blockDim.x + threadIdx.x;
  if (point_index >= num_points) {
    return;
  ...
  if (!radius_idx || !azimuth_idx || !elevation_idx) {
    ...
    voxel_indices[point_index] = nullopt;
    return;
  }
...
__global__ void subtract_left_optional_kernel(
  const DeviceOptional<int> * __restrict__ input_array, const size_t array_length,
...
__global__ void classify_point_by_return_type_and_intensity_kernel(
  ...
  const DeviceOptional<int> * __restrict__ point_indices,
  const int * __restrict__ voxel_indices,
  const DeviceOptional<int> * __restrict__ radius_indices, const double radial_resolution_m,
  ...
  const CudaPolarVoxelOutlierFilterParameters & params, const PolarDataType polar_type)
{
  ...
  CudaPooledUniquePtr<DeviceOptional<int>> point_indices = nullptr;
  ...
  // First pass: count points in each polar voxel using pre-computed coordinates
  {
    auto [radius_idx, azimuth_idx, elevation_idx, polar_voxel_indices] =
      generate_field_data_composer<DeviceOptional<int32_t>>(num_points, stream_, mem_pool_);
...
std::tuple<int, CudaPooledUniquePtr<DeviceOptional<int>>, CudaPooledUniquePtr<int>>
CudaPolarVoxelOutlierFilter::calculate_voxel_index(
  const FieldDataComposer<DeviceOptional<int32_t> *> & polar_voxel_indices,
  const size_t & num_points)
{
  ...
  for (const auto & i : FieldDataIndex()) {
    ...
    cub::TransformInputIterator<int, NulloptToMax, DeviceOptional<int> *>
    ...
    cub::TransformInputIterator<int, NulloptToLowest, DeviceOptional<int> *>
  ...
  auto point_indices =
    autoware::cuda_utils::make_unique<DeviceOptional<int>>(num_points, stream_, mem_pool_);
  ...
  {
    // Sort indices (DeviceOptional keys) using thrust::sort_by_key instead of cub::DeviceMergeSort
    auto exec = thrust::cuda::par.on(stream_);
    thrust::sort_by_key(
      exec, voxel_indices_raw.get(), voxel_indices_raw.get() + num_points, point_indices.get(),
      DeviceOptionalLess<int>{});
