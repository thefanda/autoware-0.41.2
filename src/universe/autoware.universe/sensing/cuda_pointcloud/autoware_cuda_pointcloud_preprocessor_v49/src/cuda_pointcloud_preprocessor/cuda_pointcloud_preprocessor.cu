...
#include "autoware/cuda_pointcloud_preprocessor_v49/organize_kernels.hpp"
#include "autoware/cuda_pointcloud_preprocessor_v49/outlier_kernels.hpp"
...
#include "autoware/cuda_pointcloud_preprocessor_v49/undistort_kernels.hpp"
...
void CudaPointcloudPreprocessor::setCropBoxNegative(const bool negative)
{
  crop_box_negative_ = negative;
}
  ...
  const std::uint32_t first_point_rel_stamp_nsec)
{
  ...
  if (device_crop_box_structs_.size() > 0) {
    cropBoxLaunch(
      ...
      static_cast<int>(device_crop_box_structs_.size()), crop_box_negative_, threads_per_block_,
      crop_box_blocks_per_grid, stream_);
  } else {
