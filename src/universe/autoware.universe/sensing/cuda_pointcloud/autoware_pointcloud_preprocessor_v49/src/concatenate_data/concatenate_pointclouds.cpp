...
#include "autoware/pointcloud_preprocessor_v49/concatenate_data/concatenate_pointclouds.hpp"
...
: Node("point_cloud_concatenator_component_v49", node_options)
{
  ...
  // Output Publishers
  {
    ...
    pub_output_info_ =
      this->create_publisher<autoware_sensing_msgs_v49::msg::ConcatenatedPointCloudInfo>(
...
void PointCloudConcatenationComponent::combineClouds(
  ...
  autoware_sensing_msgs_v49::msg::ConcatenatedPointCloudInfo::SharedPtr & concatenation_info_ptr)
{
  for (const auto & e : cloud_stdmap_) {
    if (e.second != nullptr) {
      ...
      if (concatenation_info_ptr == nullptr) {
        concatenation_info_ptr =
          std::make_shared<autoware_sensing_msgs_v49::msg::ConcatenatedPointCloudInfo>(
      ...
      concatenation_info_manager_->update_source_from_point_cloud(
        *transformed_cloud_ptr, e.first,
        autoware_sensing_msgs_v49::msg::SourcePointCloudInfo::STATUS_OK, *concatenation_info_ptr);
    } else {
...
void PointCloudConcatenationComponent::publish()
{
  ...
  if (concatenation_info_ptr) {
    auto output_info = std::make_unique<autoware_sensing_msgs_v49::msg::ConcatenatedPointCloudInfo>(
      *concatenation_info_ptr);
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::pointcloud_preprocessor_v49::PointCloudConcatenationComponent)
