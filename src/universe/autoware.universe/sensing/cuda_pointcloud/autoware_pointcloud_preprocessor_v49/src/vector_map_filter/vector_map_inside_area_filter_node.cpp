...
#include "autoware/pointcloud_preprocessor_v49/vector_map_filter/vector_map_inside_area_filter_node.hpp"
  ...
  const std::optional<float> & z_threshold_)
{
  ...
  for (const auto & polygon : polygons) {
    ...
    autoware::pointcloud_preprocessor_v49::utils::to_cgal_polygon(lanelet_poly, cgal_poly);
    cgal_polys.emplace_back(cgal_poly);
  }

  pcl::PointCloud<pcl::PointXYZ> filtered_cloud;
  autoware::pointcloud_preprocessor_v49::utils::remove_polygon_cgal_from_cloud(
...
: Filter("VectorMapInsideAreaFilter", node_options)
{
  ...
  // Set subscriber
  map_sub_ = this->create_subscription<autoware_map_msgs_v49::msg::LaneletMapBin>(
...
void VectorMapInsideAreaFilterComponent::mapCallback(
  const autoware_map_msgs_v49::msg::LaneletMapBin::ConstSharedPtr map_msg)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(
  autoware::pointcloud_preprocessor_v49::VectorMapInsideAreaFilterComponent)
