...
#include "negotiated_interfaces_v49/msg/negotiated_topic_info.hpp"
#include "negotiated_interfaces_v49/msg/negotiated_topics_info.hpp"
#include "negotiated_interfaces_v49/msg/supported_type.hpp"
#include "negotiated_interfaces_v49/msg/supported_types.hpp"
...
std::vector<negotiated_interfaces_v49::msg::SupportedType> default_negotiation_callback(
  ...
  size_t maximum_solutions)
{
  ...
  std::vector<negotiated_interfaces_v49::msg::SupportedType> matched_subs;
  ...
  if (upstream_negotiated_subscriptions.size() > 0) {
    ...
    for (const std::shared_ptr<UpstreamNegotiatedSubscriptionHandle> & handle :
      upstream_negotiated_subscriptions)
    {
      negotiated_interfaces_v49::msg::NegotiatedTopicsInfo topics_info =
      ...
      for (const negotiated_interfaces_v49::msg::NegotiatedTopicInfo & topic_info :
  ...
  for (size_t i = 1; i <= upstream_filtered_supported_types.size(); ++i) {
    ...
    auto check_combination =
      ...
      std::vector<std::string>::iterator last) -> bool
      {
        ...
        std::vector<negotiated_interfaces_v49::msg::SupportedType> compatible_subs;
            ...
            it != gids_needed.end(); )
          {
            ...
            for (const detail::SupportedTypeInfo & compat_info : compatible_supported_types) {
              ...
              if (std::find(key_list.begin(), key_list.end(), key) != key_list.end()) {
                negotiated_interfaces_v49::msg::SupportedType match;
...
void default_update_downstream_cb(
  ...
  upstream_negotiated_subscriptions,
  const negotiated_interfaces_v49::msg::SupportedTypes & downstream_types_to_add,
  const negotiated_interfaces_v49::msg::SupportedTypes & downstream_types_to_remove,
  ...
  negotiated_pub_options_(negotiated_pub_options)
{
  ...
  negotiated_publisher_ =
    rclcpp::create_publisher<negotiated_interfaces_v49::msg::NegotiatedTopicsInfo>(
    node_parameters_,
    node_topics_,
    topic_name_,
    rclcpp::QoS(10));
...
NegotiatedPublisher::~NegotiatedPublisher()
{
  if (upstream_negotiated_subscriptions_.size() > 0) {
    negotiated_interfaces_v49::msg::SupportedTypes downstream_types;
    for (const std::pair<std::string, detail::SupportedTypeInfo> & type : key_to_supported_types_) {
      negotiated_interfaces_v49::msg::SupportedType downstream_type;
...
void NegotiatedPublisher::supported_types_cb(
  const negotiated_interfaces_v49::msg::SupportedTypes & supported_types,
  const rclcpp::MessageInfo & msg_info)
{
  ...
  negotiated_interfaces_v49::msg::SupportedTypes downstream_types_to_add;
  negotiated_interfaces_v49::msg::SupportedTypes downstream_types_to_remove;
  ...
  if (negotiated_subscription_type_gids_->count(gid_key) > 0) {
    ...
    for (const std::string & key : negotiated_subscription_type_gids_->at(gid_key)) {
      ...
      negotiated_interfaces_v49::msg::SupportedType downstream_type_to_remove;
  ...
  for (const negotiated_interfaces_v49::msg::SupportedType & supported_type :
    supported_types.supported_types)
  {
    ...
    if (upstream_negotiated_subscriptions_.size() > 0) {
      negotiated_interfaces_v49::msg::SupportedType downstream_type_to_add;
...
void NegotiatedPublisher::start()
{
  std::string supported_type_name =
    negotiated_publisher_->get_topic_name() + std::string("/_supported_types");
  supported_types_sub_ = rclcpp::create_subscription<negotiated_interfaces_v49::msg::SupportedTypes>(
    node_parameters_,
    node_topics_,
    supported_type_name,
...
void NegotiatedPublisher::negotiate()
{
  ...
  if (matched_subs.empty()) {
    ...
    for (const negotiated_interfaces_v49::msg::SupportedType & type : matched_subs) {
      ...
      negotiated_interfaces_v49::msg::NegotiatedTopicInfo info;
...
const negotiated_interfaces_v49::msg::NegotiatedTopicsInfo &
